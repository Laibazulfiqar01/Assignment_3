...
searchDonorByName() {
  ...
  List searchDonors =
      bloodDonors.where((donor) => donor['name'].contains(donorName)).toList();
...
updateDonor() {
  ...
  int donorNumber =
      bloodDonors.indexWhere((donor) => donor["name"] == updatedName);
...
viewDonorsByBloodGroup() {
  ...
  List donorsByBloodGroup =
      bloodDonors.where((donor) => donor['bloodGroup'] == bloodGroup).toList();
...
bloodDonorByCity() {
  ...
  String city = stdin.readLineSync()!;
  List<dynamic> donorCity =
      bloodDonors.where((donor) => donor['city'] == city).toList();
  if (city.isEmpty) {
